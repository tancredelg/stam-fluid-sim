...
class HelloWorld(mglw.WindowConfig):
    ...
    gl_version = (3, 3)
    title = "Stable Fluid Simulation - Tancrède Lamort de Gail - 261085629"
    ...
    def diffuse(self, q0, kappa, dt, boundary_type):
        ...
        for k in range(self.iterations):
            q[1:self.ny+1, 1:self.nx+1] = (q0[1:self.ny+1, 1:self.nx+1] +
                                           a * (q[1:self.ny+1, :self.nx] + q[1:self.ny+1, 2:self.nx+2] +
                                                q[:self.ny, 1:self.nx+1] + q[2:self.ny+2, 1:self.nx+1])) / (1 + 4*a)
    ...
    def advect_particles(self, dt):
        ...
        # Convert all particle coordinates to grid indices at once
        ...
        new_pos = self.particles + dt * v
        self.particles = np.clip(new_pos, [self.xl, self.yl], [self.xh, self.yh])
    ...
    def advect(self, q0, dt, gv, boundary_type):
        ...
        # Perform a linear backtrace, as in Stam's GDC 2003 paper, but vectorized
        dt0x, dt0y = dt * self.nx, dt * self.ny

        # Build 'index matrices' to represent the nested for loops
        J = np.tile(np.arange(1, self.nx+1), (self.ny, 1))		# " for j in range(1, self.nx+1) "
        I = np.tile(np.arange(1, self.ny+1), (self.nx, 1)).T	# " for i in range(1, self.ny+1) "

        # Compute x and y positions using vectorized operations
        x = J - dt0x * gv[1:self.ny+1, 1:self.nx+1, 0]
        y = I - dt0y * gv[1:self.ny+1, 1:self.nx+1, 1]
        ...
        # Compute indices
        i0, j0 = np.int_(np.floor(y)), np.int_(np.floor(x))
        i1, j1 = i0 + 1, j0 + 1
        ...
        # Interpolate q using vectorized operations
        q[1:self.ny+1, 1:self.nx+1] = (s0 * (t0 * q0[i0,j0] + t1 * q0[i0,j1]) +
    ...
    def project(self):
        ...
        # Possion solve, as in Stam's GDC 2003 paper, but vectorized
        hx, hy = 1 / self.nx, 1 / self.ny

        div = np.zeros_like(self.curr_tp)
        div[1:self.ny+1, 1:self.nx+1] = \
            -0.5 * (hx * (self.curr_v[1:self.ny+1, 2:self.nx+2, 0] - self.curr_v[1:self.ny+1, :self.nx, 0]) +
                    hy * (self.curr_v[2:self.ny+2, 1:self.nx+1, 1] - self.curr_v[:self.ny, 1:self.nx+1, 1]))

        div = self.set_boundary(div, None)
        p = np.zeros_like(self.curr_tp)

        for k in range(self.iterations):
            p[1:self.ny+1, 1:self.nx+1] = (div[1:self.ny+1, 1:self.nx+1] +
                                           p[1:self.ny+1, :self.nx] +
                                           p[1:self.ny+1, 2:self.nx+2] +
                                           p[:self.ny, 1:self.nx+1] +
                                           p[2:self.ny+2, 1:self.nx+1]) / 4
            p = self.set_boundary(p, None)

        self.curr_v[1:self.ny+1, 1:self.nx+1, 0] -= 0.5 * (p[1:self.ny+1, 2:self.nx+2] - p[1:self.ny+1, :self.nx]) / hx
        self.curr_v[1:self.ny+1, 1:self.nx+1, 1] -= 0.5 * (p[2:self.ny+2, 1:self.nx+1] - p[:self.ny, 1:self.nx+1]) / hy
